...
int OpenAI::audio_post(const String& endpoint, const String& jsonBody) {
  ...
  int totalBytesRead = -1; // 记录总共读取的字节数

  if (httpCode != HTTP_CODE_OK) {
      log_e("HTTP_ERROR: %d", httpCode);
  } else {
      WiFiClient* stream = http.getStreamPtr();
      uint8_t buffer[1024]; // 缓冲区大小
      int tempTime = 0;
      while (http.connected()) {
          ...
          if (tempSize) {
              tempTime = 0; // 重置超时计时器
              size_t bytesToRead = std::min(tempSize, sizeof(buffer)); // 每次最多读取缓冲区大小
...
static cJSON * createChatMessageWithImageInput (cJSON * messages, const char * role, const char * content, const uint8_t * imageData, long unsigned int imageLength){
  ...
  // add the type image
  if(cJSON_AddStringToObject(contentObjectImage, "type", "image_url") == NULL){
    cJSON_Delete(message);
    cJSON_Delete(contentArray);
  ...
  char *printedJson = cJSON_Print(message);
  return message;
}
...
OpenAI_StringResponse OpenAI_ChatCompletion::message(String p,const uint8_t * imageData, long unsigned int imageLength, bool save){
  ...
  if(strcmp(model, "gpt-4o-mini") != 0 || imageData == NULL || imageLength == 0){
...
OpenAI_TTS::OpenAI_TTS(OpenAI &openai)
  : oai(openai)
{}
static cJSON * createTTSInput (cJSON * messages,const char * input_text, const char *response_format){
  cJSON_AddStringToObject(messages, "model", "tts-1");
  cJSON_AddStringToObject(messages, "voice", "alloy");
  cJSON_AddStringToObject(messages, "input", input_text);
  cJSON_AddStringToObject(messages, "response_format", response_format);
  return messages;
}
int OpenAI_TTS::message(String p){
  String endpoint = "audio/speech";
  OpenAI_StringResponse(NULL);
  // 初始化 cJSON 构造请求体
  cJSON *message = cJSON_CreateObject();
  if (!message) { 
    log_e("cJSON Create failed\n");
    return 0;
  }
  message=createTTSInput(message,p.c_str(),"wav");

  String jsonBody = String(cJSON_Print(message));
  cJSON_Delete(message);

  return oai.audio_post(endpoint, jsonBody);
}
